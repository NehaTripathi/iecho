...
public class FindMeIncomingRequestActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.findme_out_request);
		context=this;
		setUpViews();
		Button infoButton=(Button)findViewById(R.id.findme_out_info);
		...
		try {
			new Thread(){
				public void run() {
					serviceConnection=new WebServiceConnection("FINDME_OUTGOING_PENDING_REQUEST", "http://tempuri.org/IiEchoMobileService/GetPendingFindMeRequestIncomming", "http://tempuri.org/", "GetPendingFindMeRequestIncomming", ApplicationUtility.SERVICE_URL, "customerId",  ApplicationUtility.USER_ID);
		...
		listHandler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				try {
					...
					adapter=new ListViewCustomAdapter(FindMeIncomingRequestActivity.this, fName, lName);
	...
	private void setUpViews(){
		pendingTextView=(TextView)findViewById(R.id.find_me_out_req_number_text);
		detailListView=(ListView)findViewById(R.id.find_me_out_req_list);
		homeButton=(Button)findViewById(R.id.find_me_out_req_home_btn);
	}
	...
	private void afterResponse(){
		try {
			...
			System.out.println("inside afterResponse>>>>>");
	...
	class ListViewCustomAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				holder.imgViewAccept.setVisibility(View.VISIBLE);
				holder.imgViewReject.setVisibility(View.VISIBLE);
					...
					if(new ApplicationUtility(context).checkNetworkConnection()){
						...
						button1.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								...
								new Thread(){
									public void run() {
										serviceConnection=new WebServiceConnection("DECLINE_FIND_ME_REQUEST","http://tempuri.org/IiEchoMobileService/DeclineFindMeRequest", "http://tempuri.org/", "DeclineFindMeRequest", ApplicationUtility.SERVICE_URL, "customerId", "friendId", ApplicationUtility.USER_ID, userId[position]);
										serviceConnection=new WebServiceConnection("FINDME_OUTGOING_PENDING_REQUEST", "http://tempuri.org/IiEchoMobileService/GetPendingFindMeRequestIncomming", "http://tempuri.org/", "GetPendingFindMeRequestIncomming", ApplicationUtility.SERVICE_URL, "customerId",  ApplicationUtility.USER_ID);
			...
			holder.imgViewAccept.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(new ApplicationUtility(context).checkNetworkConnection()){
						...
						new Thread(){
							public void run() {
								serviceConnection=new WebServiceConnection("ACCEPT_FIND_ME_REQUEST","http://tempuri.org/IiEchoMobileService/AcceptFindMeRequest", "http://tempuri.org/", "AcceptFindMeRequest", ApplicationUtility.SERVICE_URL, "customerId", "friendId", ApplicationUtility.USER_ID, userId[position]);
								serviceConnection=new WebServiceConnection("FINDME_OUTGOING_PENDING_REQUEST", "http://tempuri.org/IiEchoMobileService/GetPendingFindMeRequestIncomming", "http://tempuri.org/", "GetPendingFindMeRequestIncomming", ApplicationUtility.SERVICE_URL, "customerId",  ApplicationUtility.USER_ID);
